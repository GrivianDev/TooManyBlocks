...
public:
    DirectionalLight(const glm::vec3& color, float intensity)
        : Light(LightType::Directional, color, intensity, 0.0f),
          ...
          m_shadowCasterDistance(100.0f) {}
    virtual ~DirectionalLight() = default;

    inline void setCascadeCount(unsigned int cascadeCount) {
        m_cascadeCount = std::min(cascadeCount, MAX_SHADOW_CASCADES);
    }
    ...
    inline void setShadowCasterDistance(float distance) { m_shadowCasterDistance = distance; }
    ...
    // Distance beyond each cascade towards the light to include potential shadow casters
    inline float getShadowCasterDistance() const { return m_shadowCasterDistance; }
